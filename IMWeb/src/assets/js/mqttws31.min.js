...
Paho.MQTT = (function(u) {
    ...
    function m(a) {
        for (var b = 0, c = 0; c < a.length; c++) {
            ...
            h > 2047 ? (h >= 55296 && h <= 56319 && (c++, b++), b += 3) : h > 127 ? b += 2 : b++
        }
        return b
    }
    ...
    function F(a, b, c) {
        for (var h = 0; h < a.length; h++) {
            ...
            e <= 127 ? b[c++] = e : (e <= 2047 ? b[c++] = e >> 6 & 31 |
                192 : (e <= 65535 ? b[c++] = e >> 12 & 15 | 224 : (b[c++] = e >> 18 & 7 | 240, b[c++] = e >> 12 & 63 | 128), b[c++] = e >> 6 & 63 | 128), b[c++] = e & 63 | 128)
        }
        return b
    }
    ...
    function G(a, b, c) {
        for (var h = '', e, d = b; d < b + c;) {
            ...
            if (!(e < 128)) {
                var p = a[d++] - 128
                if (p < 0) throw Error(f(g.MALFORMED_UTF, [e.toString(16), p.toString(16), '']))
                if (e < 224) e = 64 * (e - 192) + p
                else {
                    var t = a[d++] - 128
                    if (t < 0) throw Error(f(g.MALFORMED_UTF, [e.toString(16), p.toString(16), t.toString(16)]))
                    if (e < 240) e = 4096 * (e - 224) + 64 * p + t
                    else {
                        var l = a[d++] - 128
                        if (l < 0) throw Error(f(g.MALFORMED_UTF, [e.toString(16), p.toString(16), t.toString(16), l.toString(16)]))
                        if (e < 248) e = 262144 * (e - 240) + 4096 * p + 64 * t + l
                        else throw Error(f(g.MALFORMED_UTF, [e.toString(16), p.toString(16), t.toString(16), l.toString(16)]))
                    }
                }
            }
            e > 65535 && (e -= 65536, h += String.fromCharCode(55296 + (e >> 10)), e = 56320 + (e & 1023))
    ...
    n.prototype.encode = function() {
        ...
        l[0] = a
        l.set(d, 1)
        if (this.type == 3) f = r(this.payloadMessage.destinationName, h, l, f)
        else if (this.type == 1) {
            switch (this.mqttVersion) {
                case 3:
                    l.set(B, f)
    ...
    var H = function(a, b, c) {
            ...
            this.reset = function() {
                ...
                this._keepAliveInterval > 0 && (this.timeout = setTimeout(e(this), this._keepAliveInterval))
            }
        ...
        k = function(a, b, c, h, e) {
            if (!('WebSocket' in u && u.WebSocket !== null)) throw Error(f(g.UNSUPPORTED, ['WebSocket']))
            if (!('localStorage' in u && u.localStorage !== null)) throw Error(f(g.UNSUPPORTED, ['localStorage']))
            if (!('ArrayBuffer' in u && u.ArrayBuffer !== null)) throw Error(f(g.UNSUPPORTED, ['ArrayBuffer']))
            ...
            for (var d in localStorage) {
                d.indexOf('Sent:' + this._localKey) !=
                    0 && d.indexOf('Received:' + this._localKey) != 0 || this.restore(d)
            }
        }
    k.prototype.host
    k.prototype.port
    ...
    k.prototype._doConnect = function(a) {
        ...
        this.connected = !1
        this.socket = this.connectOptions.mqttVersion < 4 ? new WebSocket(a, ['mqttv3.1']) : new WebSocket(a, ['mqtt'])
    ...
    k.prototype.store = function(a, b) {
        ...
        switch (b.type) {
            case 3:
                ...
                c.payloadMessage = {}
                for (var h = '', e = b.payloadMessage.payloadBytes, d = 0; d < e.length; d++) h = e[d] <= 15 ? h + '0' + e[d].toString(16) : h + e[d].toString(16)
                ...
                a.indexOf('Sent:') == 0 && (void 0 === b.sequence && (b.sequence = ++this._sequence), c.sequence = b.sequence)
                break
            default:
    ...
    k.prototype.restore = function(a) {
        ...
        switch (c.type) {
            case 3:
                for (var b = c.payloadMessage.payloadHex,
                        e = new ArrayBuffer(b.length / 2), e = new Uint8Array(e), d = 0; b.length >= 2;) {
        ...
        a.indexOf('Sent:' + this._localKey) == 0 ? (h.payloadMessage.duplicate = !0, this._sentMessages[h.messageIdentifier] =
            h) : a.indexOf('Received:' + this._localKey) == 0 && (this._receivedMessages[h.messageIdentifier] = h)
    }
                        ...
                        switch (l) {
                            ...
                            case 3:
                                var k = z >> 1 & 3,
                                    ...
                                    u = G(e, d, r),
                                    d = d + r
                                k > 0 && (w.messageIdentifier = 256 * e[d] + e[d + 1], d += 2)
        ...
        try {
            switch (a.type) {
                case 2:
                    ...
                    if (a.returnCode === 0) this.connected = !0, this.connectOptions.uris && (this.hostIndex = this.connectOptions.uris.length)
                    else {
                    ...
                    for (var d = a.length; e < d; e++) {
                        if (c = a[e], c.type == 3 && c.pubRecReceived) {
                            var k = new n(6, {
    ...
    k.prototype._disconnected = function(a, b) {
        ...
        this._msg_queue = []
        this._notify_msg_sent = {}
        this.socket && (this.socket.onopen = null, this.socket.onmessage = null, this.socket.onerror = null, this.socket.onclose = null, this.socket.readyState === 1 && this.socket.close(), delete this.socket)
        ...
        } else if (this.connectOptions.mqttVersion === 4 && !1 === this.connectOptions.mqttVersionExplicit) this._trace('Failed to connect V4, dropping back to V3'), this.connectOptions.mqttVersion = 3, this.connectOptions.uris ? (this.hostIndex = 0, this._doConnect(this.connectOptions.uris[0])) : this._doConnect(this.uri)
    ...
    k.prototype._trace = function() {
        if (this.traceFunction) {
            for (var a in arguments) typeof arguments[a] !== 'undefined' && (arguments[a] = JSON.stringify(arguments[a]))
        ...
        if (this._traceBuffer !== null) {
            ...
            for (var b = arguments.length; a < b; a++) {
                this._traceBuffer.length ==
                    this._MAX_TRACE_ENTRIES && this._traceBuffer.shift(), a === 0 ? this._traceBuffer.push(arguments[a]) : typeof arguments[a] === 'undefined' ? this._traceBuffer.push(arguments[a]) : this._traceBuffer.push('  ' + JSON.stringify(arguments[a]))
            }
        }
    }
    ...
    var I = function(a, b, c, h) {
        var e
        if (typeof a !== 'string') throw Error(f(g.INVALID_TYPE, [typeof a, 'host']))
        if (arguments.length == 2) {
            ...
            arguments.length == 3 && (h = c, c = '/mqtt')
            if (typeof b !== 'number' || b < 0) throw Error(f(g.INVALID_TYPE, [typeof b, 'port']))
            if (typeof c !== 'string') throw Error(f(g.INVALID_TYPE, [typeof c, 'path']))
            e = 'ws://' + (a.indexOf(':') != -1 && a.slice(0, 1) != '[' && a.slice(-1) != ']' ? '[' + a + ']' : a) + ':' + b + c
        }
        ...
        if (typeof h !== 'string' || d > 65535) throw Error(f(g.INVALID_ARGUMENT, [h, 'clientId']))
        ...
        this._setOnConnectionLost = function(a) {
            if (typeof a === 'function') l.onConnectionLost = a
        ...
        this._setOnMessageDelivered = function(a) {
            if (typeof a === 'function') l.onMessageDelivered = a
        ...
        this._setOnMessageArrived = function(a) {
            if (typeof a === 'function') l.onMessageArrived = a
        ...
        this._setTrace = function(a) {
            if (typeof a === 'function') l.traceFunction = a
        ...
        this.connect = function(a) {
            ...
            if (a.mqttVersion > 4 || a.mqttVersion < 3) throw Error(f(g.INVALID_ARGUMENT, [a.mqttVersion, 'connectOptions.mqttVersion']))
            ...
            if (a.willMessage) {
                ...
                if (typeof a.willMessage.destinationName === 'undefined') throw Error(f(g.INVALID_TYPE, [typeof a.willMessage.destinationName, 'connectOptions.willMessage.destinationName']))
            }
            typeof a.cleanSession === 'undefined' && (a.cleanSession = !0)
            if (a.hosts) {
                if (!(a.hosts instanceof Array)) {
                    throw Error(f(g.INVALID_ARGUMENT, [a.hosts,
                        'connectOptions.hosts'
                    ]))
                }
                if (a.hosts.length < 1) throw Error(f(g.INVALID_ARGUMENT, [a.hosts, 'connectOptions.hosts']))
                for (var b = !1, d = 0; d < a.hosts.length; d++) {
                    if (typeof a.hosts[d] !== 'string') throw Error(f(g.INVALID_TYPE, [typeof a.hosts[d], 'connectOptions.hosts[' + d + ']']))
                    if (/^(wss?):\/\/((\[(.+)\])|([^\/]+?))(:(\d+))?(\/.*)$/.test(a.hosts[d])) {
                        ...
                        throw Error(f(g.INVALID_ARGUMENT, [a.hosts[d], 'connectOptions.hosts[' +
                            d + ']'
                        ]))
                    }
                }
                if (b) a.uris = a.hosts
                    ...
                    for (d = 0; d < a.hosts.length; d++) {
                        if (typeof a.ports[d] !== 'number' || a.ports[d] < 0) throw Error(f(g.INVALID_TYPE, [typeof a.ports[d], 'connectOptions.ports[' + d + ']']))
                        var b = a.hosts[d],
                            h =
                            a.ports[d]
                        e = 'ws://' + (b.indexOf(':') != -1 ? '[' + b + ']' : b) + ':' + h + c
                        a.uris.push(e)
                    }
                }
            }
        ...
        this.subscribe = function(a, b) {
            if (typeof a !== 'string') throw Error('Invalid argument:' + a)
            b = b || {}
            A(b, {
                qos: 'number',
                invocationContext: 'object',
            ...
            if (typeof b.qos !== 'undefined' && b.qos !== 0 && b.qos !== 1 && b.qos !== 2) {
                throw Error(f(g.INVALID_ARGUMENT, [b.qos,
        ...
        this.send = function(a, b, c, d) {
            var e
            if (arguments.length == 0) throw Error('Invalid argument.length')
            if (arguments.length == 1) {
                if (!(a instanceof x) &&
                    typeof a !== 'string') throw Error('Invalid argument:' + typeof a)
                e = a
                if (typeof e.destinationName === 'undefined') throw Error(f(g.INVALID_ARGUMENT, [e.destinationName, 'Message.destinationName']))
            } else e = new x(b), e.destinationName = a, arguments.length >= 3 && (e.qos = c), arguments.length >= 4 && (e.retained = d)
            l.send(e)
        }
    ...
    var x = function(a) {
        var b
        if (typeof a === 'string' || a instanceof ArrayBuffer || a instanceof Int8Array || a instanceof Uint8Array || a instanceof Int16Array || a instanceof Uint16Array || a instanceof Int32Array || a instanceof Uint32Array || a instanceof Float32Array || a instanceof Float64Array) b = a
        else throw f(g.INVALID_ARGUMENT, [a, 'newPayload'])
        this._getPayloadString = function() {
            return typeof b === 'string' ? b : G(b, 0, b.length)
        }
